...
ApiHandler.prototype.onItemSubmitted = function(response) {					
    ...
    if(window.parameter["width"] == 600){                                                          
          ...
          jqversion(".ligertail_widget #ligertail_widget_header").after('<div style="display:block;" class="ligertail_widget_content" id="' + item.id + '"><img src="http://g.etfv.co/' + encodeURIComponent(item.url) + '?defaulticon=lightpng" width="16" height="16" border="0" alt="favicon" /><div class="ligertail_widget_text"><span class="ligertail_widget_title"><a target="_blank" href="' + checkLink(item.url) + '">' + item.title + '</a></span></div><div class="ligertail_widget_close"><a target="_blank" href="' + url + '"><img src="' + LTDOMAIN + '/frontend/images/button_close_pay' + suffix + '.png" alt="Delete" width="12" height="12" border="0" /></a></div></div>');
    }													
}
...
ApiHandler.prototype.onGetOrderedItems = function(response) {
	...
	jqversion.each(response.items, function(i, item){
		...
		if(window.parameter["width"] == 600){
			...
			content += '<div class="ligertail_widget_content" id="' + item_obj.id + '"><img src="http://g.etfv.co/' + encodeURIComponent(item_obj.url) + '?defaulticon=lightpng" width="16" height="16" border="0" alt="favicon" /><div class="ligertail_widget_text"><span class="ligertail_widget_title"><a target="_blank" href="' + checkLink(item_obj.url) + '">' + item_obj.title + '</a></span></div><div class="ligertail_widget_close" id="' + window.LIGERTAIL_ITEMS_LOADED + '"></div></div>';
		}
